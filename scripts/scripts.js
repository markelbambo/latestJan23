...
$(document).on('taphold', '#deviceSubCisco', function(event) {
...
$(document).on('taphold', '#testToolSubIxia', function(event) {
	loading("show");
	setTimeout(function(){
...
*/
	...
	$(document).on('taphold', '#connectivitySubL1', function(event) {
	...
	$(document).on('taphold', '#connectivitySubL2', function(event) {
        if(devicesArr.length > 1){
            $(this).addClass('animated bounce');
            var id = $(this).attr("id");
            setTimeout(function(){
...
/*		    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, gotFS, fail);
		    ...
		    function gotFS(fileSystem) {
        		fileSystem.root.getFile(confName+".xml", {create: true, exclusive: false}, gotFileEntry, fail);
		    }
		    ...
		    function gotFileWriter(writer) {
        		writer.onwriteend = function(evt) {
					$.mobile.changePage($('#QRCodeDialog'),{
        		        changeHash : false,
                		transition: "pop"
		            });
        		};
			...
			var cont = "https://www.dropbox.com/s/ar6emwzhke00mqx/samplexml.xml";//content.substring(0,100);
			qrcode.makeCode(cont);
		}
    });
...
$( document ).on( "pageinit", "#DMDeviceLogs", function( event ) {
});
...
$( document ).on( "pageinit", "#RMDevices", function( event ) {
	...
	$(document).on('click', '#trDevices', function(){
	});
});
...
$( document ).on( "pageinit", "#resourceManagementPage", function( event ) {
	...
	$(document).on('click','#okEditDevice', function(event){
		loadReserveEdit();
		$(document).on('click','#rmApplyExtend', function(event){			
		//	sendReserveApply();
		//	sendRequestToCGI();
		});
	});
...
function addHistory(msg) {
    var str ="<li class='textDecor'><a href='#'>"+msg+"</a></li>"
    $("#newHistory").append(str);
    $("#box").trigger("create");
}
